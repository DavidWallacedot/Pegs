...
public class Maze implements Drawable, Runnable
{
   ...
   public boolean solve()
   {
      //DO THIS// Done this ;)
      boolean solution ; 
      solution = traverse(0,0);//simply make the initial recursive call (start in the upper left of grid)
      return solution;      
   ...
   { 
      ...
      if ( row < grid.length && row >-1 &&  column >-1 && column < grid[row].length )                                                             
      {
         //test that the trial grid is not a WALL nor has been already TRIED
         if (grid[row][column] !=TRIED && grid[row][column] !=0)
         {
            ...
            else
            {
               ...
               //make the below recursive calls to traverse in the any order: (unless we are done)
               //up, right, down, left
               ...
               //you will need to make use of the return value (update done)
            	...
            	done = 	traverse(row-1,column);//check up		
